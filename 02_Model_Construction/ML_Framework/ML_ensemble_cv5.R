ML_cv5 <- function (train_data, list_train_vali_Data, candidate_genes = NULL, 
    ...
    if (mode == "all") {
      ...
      if (length(rid) > 1) {
        ...
        cv.res <- cv.CoxBoost(est_dd2[, "OS.time"], 
                              est_dd2[, "OS"], as.matrix(est_dd2[, -c(1, 
                                                                      2)]), maxstepno = 500, K = 5, type = "verweij", 
                              penalty = pen$penalty)
      ...
      if (length(rid) > 1) {
        ...
        fit <- gbm(formula = Surv(OS.time, OS) ~ ., 
                   ...
                   shrinkage = 0.001, cv.folds = 5, n.cores = cores_for_parallel)
      ...
      if (length(rid) > 1) {
        ...
        x2 <- as.matrix(Surv(est_dd2$OS.time, est_dd2$OS))
        set.seed(seed)
        fit = cv.glmnet(x1, x2, nfold = 5, family = "cox", 
        ...
        repeat {
          tryCatch({
            cv.fit <- superpc.cv(fit, data, n.threshold = 20, 
                                 n.fold = 5, n.components = 3, min.features = 2, 
      ...
      if (T) {
        ...
        if (length(rid) > 1) {
          ...
          fit <- gbm(formula = Surv(OS.time, OS) ~ ., 
                     ...
                     cv.folds = 5, n.cores = cores_for_parallel)
          best <- which.min(fit$cv.error)
          set.seed(seed)
          fit <- gbm(formula = Surv(OS.time, OS) ~ ., 
                     data = est_dd2, distribution = "coxph", 
      ...
      cv.res <- cv.CoxBoost(est_dd[, "OS.time"], est_dd[, 
                            ...
                            K = 5, type = "verweij", penalty = pen$penalty)
      ...
      fit <- gbm(formula = Surv(OS.time, OS) ~ ., data = est_dd, 
                 ...
                 n.minobsinnode = 10, shrinkage = 0.001, cv.folds = 5, 
      ...
      fit = glmnet(x1, x2, family = "cox", alpha = 0, 
                   lambda = NULL)
      cv.fit = cv.glmnet(x1, x2, nfold = 5, family = "cox", 
                         type.measure = "class")
      rs <- lapply(val_dd_list, function(x) {
        cbind(x[, 1:2], RS = as.numeric(predict(fit, 
