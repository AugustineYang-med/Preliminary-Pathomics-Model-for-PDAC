...
list_train_vali_Data <- list(PKUFH_Training_Dataset = PKUFH_Train_dataset_only_clinical,
                             PKUFH_Test_Dataset = PKUFH_Test_dataset_only_clinical,
                             TCGA_Test_Dataset = TCGA_Test_dataset_only_clinical)
# View list structure
str(list_train_vali_Data)

col <- colnames(TCGA_Test_dataset_only_clinical)
col_features <- col[col != c("ID","Var")]
# 5. Important!!!: Model Construction
library(Mime1)

res_recurrence_only_clinical_data <- ML.Dev.Pred.Category.Sig(train_data = list_train_vali_Data$PKUFH_Training_Dataset,
...
auc_vis_category_all(res_recurrence_only_clinical_data,
...
pdf("./Result/Plot/AUC_Recurrence_only_clinical_data_c.pdf", width = 20, height = 16)  # Adjust width and height as needed
plot_list<-list()
methods <- c('nb','svmRadialWeights','rf','kknn','adaboost','LogitBoost','cancerclass')
for (i in methods) {
  plot_list[[i]]<-roc_vis_category(res_recurrence_only_clinical_data,model_name = i,dataset = c("PKUFH_Training_Dataset","PKUFH_Test_Dataset", "TCGA_Test_Dataset"),
                                   order= c("PKUFH_Training_Dataset","PKUFH_Test_Dataset", "TCGA_Test_Dataset"),
...
save(res_recurrence_only_clinical_data, file = "./Result/RData/Recurrence_Model_only_clinicaldata.RData")
